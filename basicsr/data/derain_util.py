...
def get_rain_blur(noise, length=10, angle=0, width=1):
    """
    将噪声加上运动模糊以模仿雨滴
    >>> 输入:
...
def add_rain(img, rain_type, alpha=1):
    """
    在一张图中加入雨
    >>> 输入:
        img: 图像
    ...
    rain_cfg = rain_cfg_dict[rain_type]
    ...
    noise = get_noise(img, amount=amount)
    rain = get_rain_blur(noise, length=length, angle=angle, width=width)
    img_rain = cv2.addWeighted(img, 1, rain, alpha, 1)
